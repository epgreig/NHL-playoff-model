...
cv_rf_reg <- function(data, nfolds=10, nruns=24, ntrees=4000, mtry=floor(ncol(df)/3), nodesize=5) {
  set.seed(0)
  n <- nrow(data)
  results <- matrix(nrow=nruns, ncol=4)
...
cv_rf_class <- function(data, nfolds=10, nruns=24, ntrees=4000, mtry=floor(sqrt(ncol(df))), nodesize=1) {
  set.seed(0)
  n <- nrow(data)
  results <- matrix(nrow=nruns, ncol=4)
  ...
  for (i in 1:nruns) {
    ...
    for (j in 1:nfolds) {
      ...
      fold_model <- suppressWarnings(randomForest(`Home_Won` ~ .,data=train_data, ntree=ntrees))
