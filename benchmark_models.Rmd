...
cv_rf_reg <- function(data, nfolds=10, nruns=16, mtry=floor(sqrt(ncol(df))), nodesize=5) {
  set.seed(0)
  n <- nrow(data)
  results <- matrix(nrow=nruns, ncol=4)
  ...
  for (i in 1:nruns) {
    ...
    for (j in 1:nfolds) {
      ...
      fold_model <- suppressWarnings(randomForest(`Home_Won` ~ .,data=train_data, ntree=10000, mtry=mtry, nodesize=nodesize))
      test_data$fold_prediction <- predict(fold_model, subset(test_data, select=-Home_Won), type='response')
