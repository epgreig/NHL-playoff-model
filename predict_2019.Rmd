...
EH_55 <- EH_55[, c('Team', 'Season', 'CF_60_55', 'CA_60_55', 'GF_60_55', 'GA_60_55', 'xGF_60_55', 'xGA_60_55', 'Sh%_55', 'Sv%_55')]
EH_55$`CF%_55` <- round(100* EH_55$`CF_60_55`/(EH_55$`CF_60_55` + EH_55$`CA_60_55`), 2)
EH_55$`GF%_55` <- round(100* EH_55$`GF_60_55`/(EH_55$`GF_60_55` + EH_55$`GA_60_55`), 2)
EH_55$`xGF%_55` <- round(100* EH_55$`xGF_60_55`/(EH_55$`xGF_60_55` + EH_55$`xGA_60_55`), 2)

EH_all <- read.csv("input_csvs/EH_all_08-19.csv", check.names=FALSE)
EH_all$Season <- strtoi(substr(EH_all$Season, 5, 8))
EH_all <- EH_all[, c('Team', 'Season', 'CF_60_all', 'CA_60_all', 'GF_60_all', 'GA_60_all', 'xGF_60_all', 'xGA_60_all', 'Sh%_all', 'Sv%_all')]
EH_all$`CF%_all` <- round(100* EH_all$`CF_60_all`/(EH_all$`CF_60_all` + EH_all$`CA_60_all`), 2)
EH_all$`GF%_all` <- round(100* EH_all$`GF_60_all`/(EH_all$`GF_60_all` + EH_all$`GA_60_all`), 2)
EH_all$`xGF%_all` <- round(100* EH_all$`xGF_60_all`/(EH_all$`xGF_60_all` + EH_all$`xGA_60_all`), 2)
...
stats <- c('CF%_55', 'CF_60_55', 'CA_60_55', 'GF%_55', 'GF_60_55', 'GA_60_55', 'xGF%_55', 'xGF_60_55', 'xGA_60_55', 'Sh%_55', 'Sv%_55', 'CF%_all', 'CF_60_all', 'CA_60_all', 'GF%_all', 'ENGD', 'GF_60_all', 'GA_60_all', 'xGF%_all', 'xGF_60_all', 'xGA_60_all', 'Sh%_all', 'Sv%_all', 'HDCF%_55', 'Time_Led', 'PP%', 'PK%', 'W', 'ROW', 'Berg')
...
```{r}
dStats_5 <- c("dCF_60_55", "dGF._55","dGF._all", "dxGF._all",  "dPK.")
```
...
predict_round <- function(series_data, year, round=0) {
  ...
  train <- prior_series[,c('Home_Won', dStats_5)]
  train_reversed <- -train
  train_reversed$Home_Won <- 1 - train$Home_Won
  train <- rbind(train, train_reversed)
  ...
  rf_5 <- suppressWarnings(randomForest(`Home_Won` ~ ., data=train, ntree=50000, mtry=1, nodesize=44))
