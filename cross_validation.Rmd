...
cv_rf_reg <- function(dataset, nfolds=10, nruns=32, ntrees=4000, maxnodes=NULL, mtry=floor(ncol(df)/3), nodesize=5, symm=FALSE) {
  set.seed(0)
  n <- nrow(dataset)
  results <- matrix(nrow=nruns, ncol=4)
  ...
  for (i in 1:nruns) {
    ...
    for (j in 1:nfolds) {
      ...
      fold_model <- suppressWarnings(randomForest(`Home_Won` ~ .,data=train_data, ntree=ntrees, mtry=mtry, nodesize=nodesize, maxnodes=NULL))
      test_data$fold_prediction <- predict(fold_model, subset(test_data, select=-Home_Won), type='response')
...
cv_rf_class <- function(dataset, nfolds=10, nruns=32, ntrees=4000, mtry=floor(sqrt(ncol(df))), nodesize=1, symm=FALSE) {
  set.seed(0)
  n <- nrow(dataset)
  results <- matrix(nrow=nruns, ncol=4)
