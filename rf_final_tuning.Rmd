...
df_7 <- df[,c('Home_Won', dStats_clas4)]
df_7_class <- df_7
...
#rf_grid(df_7_class, mtrys=1, nodesizes=15:35, ntrees=3000)
...
# Grids attempted:
#rf_grid(df_4_class, mtrys=1:3, nodesizes=10:60, ntrees=10000)
#rf_grid(df_4_class, mtrys=1, nodesizes=5:40, ntrees=3000)
...
#cv_rf_class(df_13, nfolds=10, nruns=64, mtry=2, nodesize=62, ntrees=1600)
# 13 FEATURES
# Log Loss: 0.656 (sd 0.11)
# Accuracy: 65.0% (sd 0.12)

#cv_rf_class(df_7, nfolds=10, nruns=32, mtry=1, nodesize=20)
# 7 FEATURES
# Log Loss: 0.687 (sd 0.13)
# Accuracy: 65.2% (sd 0.11)

#cv_rf_class(df_4, nfolds=10, nruns=32, mtry=1, nodesize=23)
# 4 FEATURES
...
## Random Forest (Regression) Performance, Tuned on 13, 7, 4 features
```{r}
# FULL FEATURES
# Log Loss: 0.652 (sd 0.08)
# Accuracy: 63% (sd 0.12)

#cv_rf_reg(df_13, nfolds=10, nruns=256, mtry=2, nodesize=62, ntrees=1600)
# 13 FEATURES
# Log Loss: 0.635 (sd 0.07)
# Accuracy: 65.8% (sd 0.12)

#cv_rf_reg(df_7, nfolds=10, nruns=256, mtry=1, nodesize=25, ntrees=1600)
# 7 FEATURES
# Log Loss: 0.635 (sd 0.09)
# Accuracy: 67.8% (sd 0.11)

#cv_rf_reg(df_4, nfolds=10, nruns=256, mtry=1, nodesize=25, ntrees=1600)
# 4 FEATURES
# Log Loss: 0.632 (sd 0.11)
# Accuracy: 68.2% (sd 0.12)
```
