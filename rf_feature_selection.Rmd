...
dStats_unpaired <- dStats[!dStats%in%c('dCF._55', 'dCF.60_all', 'dCA.60_all', 'dGF.60_55', 'dGA.60_55', 'dxGF._55', 'dxGA.60_55', 'dxGF.60_55', 'dSh._55', 'dSv._55', 'dxBerg', 'dW', 'dENGD')]
dStats_unpaired
```

```{r}
# Train a second time (with scaled-down hyperparams)  to identify more weak features
#imp_model_2 <- suppressWarnings(randomForest(`Home_Won` ~ ., data=df[,c('Home_Won', dStats_unpaired)], ntree=300000, mtry=1, nodesize=75, importance=TRUE))
...
dStats_9 <- dStats_unpaired[!dStats_unpaired%in%c('dCA.60_55', 'dGF.60_all', 'dxGF.60_all', 'dxGA.60_all', 'dSh._all', 'dSv._all', 'dHDCF._55', 'dPP.', 'dROW')]
dStats_9
```

```{r}
# Train a third time (with scaled-down hyperparams)  to identify more weak features
#imp_model_3 <- suppressWarnings(randomForest(`Home_Won` ~ ., data=df[,c('Home_Won', dStats_9)], ntree=300000, mtry=1, nodesize=75, importance=TRUE))
#print(imp_model_3$importance)
#print(cor(df[,c('Home_Won', dStats_9)]))
