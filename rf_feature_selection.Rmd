...
# IncNodePurity or MeanDecreaseGini (MDG): a feature with a low MDG indicates that when a decision tree uses that feature to split a node, it does not decrease cross entropy with the target. So a high MDG means that a feature provides at least some predictive value at some level of some trees.
...
```{r}
# Create a set of less than features with most predictive power
#huge_model_r4 <- suppressWarnings(randomForest(`Home_Won` ~ ., data=dfr4, ntree=1000000, mtry=1, nodesize=60, importance=TRUE))
#print(huge_model_r4$importance)

# Keep features with MDA>0.004 and MDG>0.9
dStats_regr5 <- c('dCF.60_55', 'dGF._55', 'dGF._all', 'dxGF._all')
dfr5 <- dfr4[,c('Home_Won', dStats_regr5)]
```
...
# Keep features with MDA>1e-03 and MDG<0.4
#dStats_clas4 <- c('dCF.60_55', 'dGF._55', 'dxGF._55', 'dGF._all', 'dxBerg', 'dxGF._all', 'dPK.')
#dfc4 <- dfc3[,c('Home_Won', dStats_clas4)]
# dStats_clas4 is the same as dStats_regr4!
```

#Result: We have three new subsets of the features: with 13, 7, and 4 features respectively.