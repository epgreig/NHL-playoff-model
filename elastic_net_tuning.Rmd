...
alphas_default <- seq(0, 0.5, len=11)^3 # default range in cva.glmnet
lambdas_default <- exp(seq(log(0.00001), log(0.001), len=99)) # equally log-spaced
...
elastic_net_grid <- function(data, alphas=alphas_default, lambdas=lambdas_default, num_iterations) {
  ...
  for (i in 1:a) {
    ...
    for (k in 1:num_iterations) {
      ...
      cvms[k,] <- temp_model$cvup
    }
    results[i,] <- colMeans(cvms)
  }
  rownames(results) <- paste('alpha', alphas, sep='=')
  colnames(results) <- paste('lambda', lambdas, sep='=')
...
# elastic_net_grid(df, alphas=seq(0.2,0.7,len=6)^3, lambdas=exp(seq(log(0.35), log(0.85), len=11)) lambdas=num_iterations=10)
```
