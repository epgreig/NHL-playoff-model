...
#elastic_net_grid(df, alphas=seq(0.96,1,len=5)^3, lambdas=exp(seq(log(0.025), log(0.045), len=11)), num_iterations=500)
elastic_net_grid(df, alphas=seq(0.96,1,len=13), lambdas=exp(seq(log(0.039), log(0.044), len=11)), num_iterations=750)

#result: alpha=0.966, lambda=0.041
#
```
...
for (i in 1:nruns) {
  ...
  for (j in 1:nfolds) {
    ...
    fold_model <- glmnet(`Home_Won` ~ .,data=train_data, family='binomial', alpha=0.966, lambda=0.041)
...
# ELASTIC NET
# Log Loss: 0.668 (sd 0.07)
# Accuracy: 59.6% (sd 0.12)
```
