...
predict_round <- function(series_data, year, round=0) {
  ...
  rf_11 <- suppressWarnings(randomForest(`Home_Won` ~ ., data=train[,c('Home_Won', dStats_11)], ntree=250000, mtry=1, nodesize=70))
  rf_5 <- suppressWarnings(randomForest(`Home_Won` ~ ., data=train[,c('Home_Won', dStats_5_uncorr)], ntree=250000, mtry=1, nodesize=70))
  rf_4 <- suppressWarnings(randomForest(`Home_Won` ~ ., data=train[,c('Home_Won', dStats_4_best)], ntree=250000, mtry=1, nodesize=40))
  ...
  output$rf_11_pred <- predict(rf_11, test, type='response')
  output$rf_11_loss <- -log(abs(1-output$Home_Won-output$rf_11_pred))
