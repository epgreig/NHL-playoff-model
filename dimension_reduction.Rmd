...
alphas <- seq(0, 1, len=11)^3 # default range in cva.glmnet
lambdas <- exp(seq(log(0.01), log(1), len=11)) # equally log-spaced
num_iterations <- 3
results <- matrix(nrow=11,ncol=11)

for (i in 1:length(alphas)) {
  for (k in 1:num_iterations) {
    temp_model <- cv.glmnet(Home_Won ~ ., data=df, alpha=alphas[i], lambda = lambdas, family='binomial')
