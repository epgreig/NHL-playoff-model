---
title: "hyperparameter_tuning.Rmd"
author: "Ethan Greig"
date: "1/20/2019"
...
hyper_params <- function(mtrys, nodesizes, ntrees=4000) {
  results <- matrix(nrow=length(mtrys), ncol=length(nodesizes))
  data <- df
  data$Home_Won <- as.factor(data$Home_Won)
  ...
  write.table(results, file="~/repos/nhl-playoff-model/output_csvs/grid_search_27_features.csv", sep=',', col.names=NA)
}

# Grids attempted:(mtrys x nodesizes)
# hyper_params(mtrys=1:25, nodesizes=1:25)
# hyper_params(mtrys=1:15, nodesizes=16:35)
# hyper_params(mtrys=1:8, nodesizes=28:60)
# hyper_params(mtrys=2:8, nodesizes=50:69)
# hyper_params(mtrys=2:8, nodesizes=50:59, ntrees=6000)
# hyper_params(mtrys=2:8, nodesizes=55:65, ntrees=10000)
...
## Tuned Random Forest (Regression) Benchmark
```{r}
cv_rf_reg(df, nfolds=10, nruns=24, mtry=6, nodesize=60)
# BENCHMARK
# Log Loss: 0.67 (sd 0.10)
# Accuracy: 63% (sd 0.12)

# AFTER TUNING
# Log Loss: 0.654 (sd 0.08)
# Accuracy: 63% (sd 0.12)
```

## Tuned Random Forest (Classification) Benchmark
```{r}
