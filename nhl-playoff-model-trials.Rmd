...
prin_data2 <- rbind(prin_data, prin_data_flip)
prin_data2 <- cbind(prin_data2, pca2$scores)

prin_data <- cbind(prin_data, pca$scores)

write.csv(pca$loadings, 'output_csvs/pca_loadings.csv')
write.csv(pca2$loadings, 'output_csvs/pca2_loadings.csv')
...
regression_binary <- function(dataset) {
  return(suppressWarnings(glm(`Home_Won` ~ `Comp.1` + `Comp.2` + `Comp.3`,data=dataset, weights=rep(0.5, nrow(dataset)), family=binomial(link='logit'))))
}
...
ten_fold_cross_validate <- function(dataset){
  dataset_flip <- dataset
  dataset_flip$Comp.1 <- -dataset_flip$Comp.1
  dataset_flip$Comp.2 <- -dataset_flip$Comp.2
  dataset_flip$Comp.3 <- -dataset_flip$Comp.3
  dataset_flip$Home_Won <- 1 - dataset_flip$Home_Won
  dataset <- rbind(dataset, dataset_flip)
  series_shuffled <- dataset[sample(nrow(dataset)),]
  folds <- cut(seq(1,nrow(series_shuffled)),breaks=10,labels=FALSE)
  ...
  return(c(mean(log_loss_binary), mean(mse_binary)))
}

ten_fold_metrics <- matrix(NA, nrow=200, ncol=2)
colnames(ten_fold_metrics) <- c("Log Loss Binary", "Mean Squared Error Binary")
for(j in 1:200) {
...
## 
##
## Bind playoff_teams_2018 with adj_data_10
...
predict_matchup <- function(home_team, away_team) {
  ...
  win_prob <- predict(model2, component_values[,Comps], type='response')
...
write.csv(predictions, 'output_csvs/predictions2_2018.csv')
```
...
pca_create <- function(dataset) {
  dStat_table <- dataset[,dStats]
  dStat_table_flip <- -dataset[,dStats]
  pca <- princomp(rbind(dStat_table, dStat_table_flip), cor=TRUE)

  prin_data <- dataset[,c('Year', 'Home', 'Away', 'Home_Won')]
  prin_data_flip <- dataset[,c('Year', 'Home', 'Away', 'Home_Won')]
  orig_home <- prin_data_flip$Home
  prin_data_flip$Home <- prin_data_flip$Away
  prin_data_flip$Away <- orig_home
  prin_data_flip$Home_Won <- 1 - prin_data_flip$Home_Won
...
pca_glm <- function(pca_dataset, comps=Comps) {
  # use comps somehow?
  model <- glm(`Home_Won` ~ `Comp.1` + `Comp.2` + `Comp.3`,data=dataset, weights=rep(0.5, nrow(dataset)), family=binomial(link='logit'))
  return(model)
}

pca_validate <- function(pca_dataset, num_comps=10) {
  pca_validations <- matrix(0,nrow=num_comps, ncol=3)
  colnames(pca_validations) <- c('Components', 'Log Loss', 'MSE')
  
  Comps <- c('Comp.1', 'Comp.3')
  pca_validations[1,1] <- Comps
  pca_validations[1,2:3] <- pca_iterate_validation(pca_dataset, Comps)
  for (num in 2:num_comps) {
    Comps <- NULL
    for (i in 1:num) {
    Comps[i] <- paste0('Comp.', i)
    }
    pca_validations[num,1] <- Comps
    pca_validations[num,2:3] <- pca_iterate_validation(pca_dataset, Comps)
  }
}
...
pca_cross_validate <- function(pca_dataset, comps) {
  ...
  return(c(mean(log_loss), mean(mse)))
}

pca_iterate_validation <- function(pca_dataset, comps) {
  iteration_errors <- matrix(NA, nrow=100, ncol=2)
  colnames(iteration_errors) <- c("Log Loss Binary", "Mean Squared Error Binary")
  for (i in 1:100) {
