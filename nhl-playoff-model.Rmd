...
```{r}
playoff_teams_2018 <- read.csv('input_csvs/playoff_teams_2018.csv', check.names=FALSE)
...
raw_data_pred <- merge(merge(corsica1_pred, corsica2_pred, by="Team"), morestats_pred, by="Team")
raw_data_pred$`AdjGF%` <- with(raw_data_pred, round(100*(allGF-GF_EN)/(allGF-GF_EN+allGA-GA_EN),2))
raw_data_pred <- raw_data_pred[, c('Team', 'Season', 'allGF%', 'AdjGF%', 'xGF%', 'CF%', 'Time Led', 'Sh%', 'Sv%', 'PP%', 'PK%', 'ROW')]
colnames(raw_data_pred) <- c('Team', 'Season', 'GF%', 'AdjGF%', '5v5_xGF%', '5v5_CF%', 'Time_Led', '5v5_Sh%', '5v5_Sv%', 'PP%', 'PK%', 'ROWins')
